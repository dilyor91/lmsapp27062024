...
type RestOf<T extends ICourse | NewCourse> = Omit<T, 'courseStartDate' | 'courseEndDate'> & {
...
export class CourseService {
  ...
  protected convertDateFromClient<T extends ICourse | NewCourse | PartialUpdateCourse>(course: T): RestOf<T> {
    return {
      ...course,
      courseStartDate: course.courseStartDate?.toJSON() ?? null,
      courseEndDate: course.courseEndDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCourse: RestCourse): ICourse {
    return {
      ...restCourse,
      courseStartDate: restCourse.courseStartDate ? dayjs(restCourse.courseStartDate) : undefined,
      courseEndDate: restCourse.courseEndDate ? dayjs(restCourse.courseEndDate) : undefined,
    };
  }
