...
import { EntityArrayResponseType, TimeTableService } from '../service/time-table.service';
...
export class TimeTableComponent implements OnInit {
  ...
  delete(timeTable: ITimeTable): void {
    const modalRef = this.modalService.open(TimeTableDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.timeTables = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITimeTable[] | null): ITimeTable[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.timeTableService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
