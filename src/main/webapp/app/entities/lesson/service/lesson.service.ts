...
type RestOf<T extends ILesson | NewLesson> = Omit<T, 'startPlanDate' | 'actualLessonDate'> & {
...
export class LessonService {
  ...
  protected convertDateFromClient<T extends ILesson | NewLesson | PartialUpdateLesson>(lesson: T): RestOf<T> {
    return {
      ...lesson,
      startPlanDate: lesson.startPlanDate?.toJSON() ?? null,
      actualLessonDate: lesson.actualLessonDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restLesson: RestLesson): ILesson {
    return {
      ...restLesson,
      startPlanDate: restLesson.startPlanDate ? dayjs(restLesson.startPlanDate) : undefined,
      actualLessonDate: restLesson.actualLessonDate ? dayjs(restLesson.actualLessonDate) : undefined,
    };
  }
