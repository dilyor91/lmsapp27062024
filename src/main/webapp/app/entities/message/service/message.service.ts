...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'senderDate'> & {
...
export class MessageService {
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      senderDate: message.senderDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      senderDate: restMessage.senderDate ? dayjs(restMessage.senderDate) : undefined,
    };
  }
